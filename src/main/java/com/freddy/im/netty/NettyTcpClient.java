...
public class NettyTcpClient implements IMSClientInterface {
    ...
    public void sendMsg(MessageProtobuf.Msg msg, boolean isJoinTimeoutManager) {
        if (msg == null || msg.getHead() == null) {
            Log.e(TAG, "发送消息失败，消息为空\tmessage=" + msg);
            return;
        }
        ...
        if (channel == null) {
            Log.e(TAG, "发送消息失败，channel为空\tmessage=" + msg);
        }

        try {
            channel.writeAndFlush(msg);
        } catch (Exception ex) {
            Log.e(TAG, "发送消息失败，reason:" + ex.getMessage() + "\tmessage=" + msg, ex);
        }
    }
        ...
        switch (connectStatus) {
            ...
            case IMSConfig.CONNECT_STATE_SUCCESSFUL: {
                ...
                if (handshakeMsg != null) {
                    Log.d(TAG, "发送握手消息，message=" + handshakeMsg);
                    sendMsg(handshakeMsg, false);
                } else {
                    Log.e(TAG, "请应用层构建握手消息！");
                }
                break;
            }
    ...
    private void removeHandler(String handlerName) {
        try {
            ...
            e.printStackTrace();
            Log.e(TAG, "移除handler失败，handlerName=" + handlerName, e);
        }
    }
