...
public class MsgTimeoutTimerManager {
    ...
    public void add(MessageProtobuf.Msg msg) {
        ...
        if (true/*msgType == handshakeMsgType || msgType == heartbeatMsgType || msgType == clientReceivedReportMsgType*/) {
            return;
        }
        ...
        System.out.println("添加消息超发送超时管理器，message=" + msgId + "\t当前管理器消息数：" + mMsgTimeoutMap.size());
    }
    ...
    public void remove(String msgId) {
        ...
        System.out.println("从发送消息管理器移除消息，message=" + msgId + "\t当前管理器消息数：" + mMsgTimeoutMap.size());
    }
    ...
    public synchronized void onResetConnected() {
        System.out.println("onResetConnected");
